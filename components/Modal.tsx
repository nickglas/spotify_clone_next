...
const Modal: React.FC<ModalProps> = ({isOpen, onchange, title, description, children}) => {
  ...
  const [toggleOpen, setToggleOpen] = useState<boolean>(isOpen)
  ...
  return (
    ...
    >
      <Dialog.Portal>
        <Dialog.Overlay
          className="bg-neutral-900/10 backdrop-blur-sm fixed inset-0"
        ...
        <Dialog.Content className="fixed drop-shadow-md border border-neutral-700 top-[50%] left-[50%] max-h-full h-full md:h-auto md:max-h-[85vh] w-full md:w-[90vw] md:max-w-[450px] translate-x-[-50%] translate-y-[-50%] rounded-md bg-neutral-800 p-[25px] focus:outline-none">
          ...
          <Dialog.Close asChild>
            <button onClick={() => {isOpen == false}} className="text-neutral-400 hover:text-white absolute top-[10px] right-[10px] inline-flex h-[25px] w-[25px] appearance-none items-center justify-center rounded-full focus:outline-none">
              <IoMdClose />
            </button>
          </Dialog.Close>
        </Dialog.Content>
